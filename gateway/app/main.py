...
def public_key():
    """Return the gateway's public key PEM. Verifiers use this to validate event signatures."""
    return {"signer_id": signer_id(), "cert_fingerprint": cert_fingerprint(), "public_key_pem": public_key_pem()}
...
def metrics():
    """Return runtime metrics for the current gateway session."""
    s = collector.summary()
...
def export_metrics():
    """Export events.csv and metrics.csv to the results directory."""
...
def register_event(body: RegisterEventRequest):
    ...
    log.info("registered event_id=%s tx_id=%s latency_ms=%.1f", event_id, tx_id, latency_ms)
...
def get_event_history(event_id: str):
    """Return Fabric write history. A legitimate record has exactly one write entry."""
    try:
...
def trace_chain(event_id: str):
    ...
    A broken link indicates selective deletion or tampering.
    """
    try:
...
def verify_event(event_id: str = Query(...), body: VerifyRequest = ...):
    """Verify a payload hash against the stored hash and validate the gateway signature.

    The caller provides the SHA-256 hash of their copy of the payload.
    The endpoint returns PASS if it matches the on-chain hash, FAIL otherwise.
    """
    try:
    ...
    log.info("verify event_id=%s result=%s sig_valid=%s", event_id, result_str, sig_valid)
...
def export_audit_report(
    ...
    """Generate a JSON audit report for a specific actor, zone, or event type."""
    try:
